    ...
    <div className="space-y-4 sm:space-y-5">
      <div className="grid gap-3 sm:gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(380px,520px)] lg:items-start">
        <div className="order-2 rounded-xl border border-slate-200 bg-white p-3 shadow-sm sm:rounded-2xl sm:p-5 lg:order-1">
          ...
          <div className="mt-3 grid grid-cols-3 gap-2 rounded-xl border border-orange-100 bg-orange-50/70 p-2.5 text-[10px] font-bold text-orange-950 sm:p-3">
            <div className="rounded-lg bg-white/80 p-2"><span className="block text-orange-600">Bước 1</span><span className="mt-0.5 block leading-4">Chọn ý tưởng listing</span></div>
            <div className="rounded-lg bg-white/80 p-2"><span className="block text-orange-600">Bước 2</span><span className="mt-0.5 block leading-4">Tạo trường khách nhập</span></div>
            <div className="rounded-lg bg-white/80 p-2"><span className="block text-orange-600">Bước 3</span><span className="mt-0.5 block leading-4">Căn vùng trên mockup</span></div>
          </div>
          ...
          <div className="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-white">
            <div className="flex items-start justify-between gap-3 border-b border-slate-200 bg-slate-50 px-3 py-2.5"><div><span className="text-[11px] font-extrabold text-slate-800">Review liên kết trước khi lưu</span><p className="mt-0.5 text-[10px] leading-4 text-slate-500">Kiểm tra từng bước khách nhập đã nối đúng vùng in và layer preview.</p></div><span className={`shrink-0 rounded-full px-2 py-1 text-[9px] font-bold ${fields.every(field => printAreas.some(area => area.fieldIds?.includes(field.id)) || (canvas?.layers || []).some(layer => layer.source === "FIELD" && layer.fieldId === field.id && printAreas.some(area => area.id === layer.printAreaId))) ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>{fields.filter(field => printAreas.some(area => area.fieldIds?.includes(field.id)) || (canvas?.layers || []).some(layer => layer.source === "FIELD" && layer.fieldId === field.id && printAreas.some(area => area.id === layer.printAreaId))).length}/{fields.length} đã nối</span></div>
            {fields.length === 0 ? <p className="p-3 text-[10px] text-slate-500">Thêm trường cá nhân hóa để bắt đầu nối với vùng in.</p> : <div className="divide-y divide-slate-100">{fields.map((field, fieldIndex) => { const directArea = printAreas.find(area => area.fieldIds?.includes(field.id)); const layer = (canvas?.layers || []).find(item => item.source === "FIELD" && item.fieldId === field.id); const layerArea = layer ? printAreas.find(area => area.id === layer.printAreaId) : undefined; const area = directArea || layerArea; return <div key={field.id} className="grid gap-2 px-3 py-2.5 sm:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)_auto] sm:items-center"><button type="button" onClick={() => setExpandedId(field.id)} className="min-h-10 min-w-0 text-left"><span className="block truncate text-[11px] font-extrabold text-slate-800">{fieldIndex + 1}. {field.label}</span><span className="block truncate text-[9px] text-slate-500">{field.step || `Bước ${fieldIndex + 1}`}{field.required ? " · bắt buộc" : " · tùy chọn"}</span></button><label className="text-[9px] font-bold text-slate-500">Vùng in<select value={area?.id || ""} onChange={event => { const nextAreaId = event.target.value; const nextAreas = printAreas.map(candidate => ({ ...candidate, fieldIds: [...new Set((candidate.fieldIds || []).filter(id => id !== field.id).concat(candidate.id === nextAreaId ? [field.id] : []))] })); const layers = [...(canvas?.layers || [])]; const layerIndex = layers.findIndex(item => item.source === "FIELD" && item.fieldId === field.id); if (nextAreaId) { if (layerIndex >= 0) layers[layerIndex] = { ...layers[layerIndex], printAreaId: nextAreaId }; else layers.push({ id: newId("layer"), label: field.label, source: "FIELD", fieldId: field.id, printAreaId: nextAreaId, sceneId: nextAreas.find(candidate => candidate.id === nextAreaId)?.sceneId, zIndex: layers.length }); } else if (layerIndex >= 0) layers.splice(layerIndex, 1); onCanvasChange({ ...(canvas || {}), printAreas: nextAreas, layers }); }} className="mt-1 min-h-10 w-full rounded-lg border border-slate-300 bg-white px-2 text-[10px]"><option value="">Chưa gán</option>{printAreas.map(areaOption => <option key={areaOption.id} value={areaOption.id}>{areaOption.label || areaOption.id}</option>)}</select></label><span className={`self-center rounded-full px-2 py-1 text-center text-[9px] font-bold ${area ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}>{area ? "Preview sẵn sàng" : "Thiếu vùng in"}</span></div>; })}</div>}
          </div>
        </div>
