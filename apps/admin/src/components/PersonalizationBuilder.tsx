...
const NumberInput = ({ label, value, onChange, min = 0, max = 100 }: { label: string; value?: number; onChange: (value: number | undefined) => void; min?: number; max?: number }) => (
  <label className="block text-[10px] font-bold text-slate-500">
    {label}
    <input type="number" min={min} max={max} value={value ?? ""} onChange={event => onChange(event.target.value === "" ? undefined : Number(event.target.value))} className="mt-1 min-h-10 w-full rounded-lg border border-slate-300 bg-white px-2 text-xs text-slate-800 outline-none focus:border-orange-500" />
  </label>
);
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <label className="inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs font-bold text-slate-700">
              <input type="checkbox" checked={enabled} onChange={event => onEnabledChange(event.target.checked)} className="h-4 w-4 rounded border-slate-300 text-orange-600 focus:ring-orange-500" />
              Bật cá nhân hoá
            </label>
          </div>
          <label className="mt-4 block text-xs font-bold text-slate-700">URL mockup nền trơn</label>
          <div className="mt-1.5 flex items-center gap-2">
            <ImageIcon className="h-4 w-4 shrink-0 text-slate-400" />
            <input type="url" value={mockupUrl || ""} onChange={event => onMockupUrlChange(event.target.value)} placeholder="https://.../mockup-tron.png" className="min-h-11 w-full rounded-xl border border-slate-300 px-3 text-xs outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20" />
          </div>
            ...
            return (
              <article key={field.id} className="overflow-hidden rounded-xl border border-slate-200 bg-white">
                <div className="flex items-center gap-2 bg-slate-50 px-3 py-2.5">
                  ...
                  <button type="button" onClick={() => setExpandedId(expanded ? null : field.id)} className="flex min-h-11 min-w-0 flex-1 items-center justify-between gap-3 text-left"><span className="min-w-0"><span className="block truncate text-xs font-extrabold text-slate-900">{field.label}</span><span className="text-[10px] font-semibold text-slate-500">{FIELD_TYPES.find(item => item.value === field.type)?.label || field.type}{field.required ? " · bắt buộc" : ""}</span></span>{expanded ? <ChevronUp className="h-4 w-4 text-slate-400" /> : <ChevronDown className="h-4 w-4 text-slate-400" />}</button>
                  ...
                  <div className="space-y-4 border-t border-slate-200 p-4">
                    ...
                    {supportsOptions(field.type) && (
                      <div className="rounded-xl border border-slate-200 p-3">
                        ...
                        <div className="space-y-2">{(field.options || []).map((option, optionIndex) => <div key={option.id} className="grid gap-2 rounded-lg bg-slate-50 p-2 sm:grid-cols-[1fr_1fr_1.5fr_auto]"><input aria-label="Tên lựa chọn" value={option.label} onChange={event => patchOption(index, optionIndex, { label: event.target.value })} placeholder="Tên hiển thị" className="min-h-10 rounded-lg border border-slate-300 px-2 text-xs" /><input aria-label="Giá trị lựa chọn" value={option.value} onChange={event => patchOption(index, optionIndex, { value: event.target.value })} placeholder={field.type === "COLOR_SWATCH" ? "#f97316" : "Giá trị"} className="min-h-10 rounded-lg border border-slate-300 px-2 text-xs" /><input aria-label="URL ảnh lựa chọn" value={option.previewAssetUrl || ""} onChange={event => patchOption(index, optionIndex, { previewAssetUrl: event.target.value })} placeholder="URL ảnh design/thumbnail" className="min-h-10 rounded-lg border border-slate-300 px-2 text-xs" /><button type="button" aria-label="Xóa lựa chọn" onClick={() => patchField(index, { options: (field.options || []).filter((_, itemIndex) => itemIndex !== optionIndex) })} className="grid h-10 w-10 place-items-center rounded-lg text-rose-600 hover:bg-rose-50"><Trash2 className="h-3.5 w-3.5" /></button></div>)}</div>
                      </div>
                    )}
