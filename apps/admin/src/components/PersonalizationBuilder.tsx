    ...
    <div className="space-y-5">
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(280px,340px)] lg:items-start">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3"><div className="mb-2 flex items-center justify-between"><span className="text-[11px] font-extrabold text-slate-800">Mặt preview / scene</span><button type="button" onClick={() => { const nextScene = { id: newId("scene"), label: `Mặt ${((canvas?.scenes || []).length || 0) + 1}`, mockupUrl }; onCanvasChange({ ...(canvas || {}), scenes: [...(canvas?.scenes || []), nextScene], printAreas }); setSelectedSceneId(nextScene.id); }} className="text-[10px] font-bold text-orange-700">+ Thêm mặt</button></div>{(canvas?.scenes || []).length === 0 ? <p className="text-[10px] text-slate-500">Có thể thêm mặt trước, mặt sau hoặc góc lifestyle; mỗi mặt có mockup và vùng in riêng.</p> : <div className="space-y-2">{(canvas?.scenes || []).map((scene, sceneIndex) => <div key={scene.id} className={`grid gap-2 rounded-lg p-1.5 sm:grid-cols-[120px_1fr_auto] ${activeSceneId === scene.id ? "bg-orange-50 ring-1 ring-orange-200" : ""}`}><input value={scene.label} onFocus={() => setSelectedSceneId(scene.id)} onChange={event => { const nextScenes = [...(canvas?.scenes || [])]; nextScenes[sceneIndex] = { ...scene, label: event.target.value }; onCanvasChange({ ...(canvas || {}), scenes: nextScenes, printAreas }); }} className="min-h-9 rounded border border-slate-300 px-2 text-[10px]" /><input aria-label={`URL mockup ${scene.label}`} value={scene.mockupUrl || ""} onFocus={() => setSelectedSceneId(scene.id)} onChange={event => { const nextScenes = [...(canvas?.scenes || [])]; nextScenes[sceneIndex] = { ...scene, mockupUrl: event.target.value }; onCanvasChange({ ...(canvas || {}), scenes: nextScenes, printAreas }); }} placeholder="URL mockup mặt này" className="min-h-9 rounded border border-slate-300 px-2 text-[10px]" /><button type="button" onClick={() => { const nextScenes = (canvas?.scenes || []).filter((_, index) => index !== sceneIndex); onCanvasChange({ ...(canvas || {}), scenes: nextScenes, printAreas }); setSelectedSceneId(nextScenes[0]?.id || null); }} className="text-[10px] font-bold text-rose-600">Xóa</button></div>)}</div>}</div>
        ...
        <div className="relative aspect-square overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 shadow-inner" onPointerMove={event => { const drag = dragRef.current; const resize = resizeRef.current; const rect = event.currentTarget.getBoundingClientRect(); const targetId = drag?.id || resize?.id; const areaIndex = targetId ? printAreas.findIndex(area => area.id === targetId) : -1; if (areaIndex < 0) return; if (drag) { const dx = (event.clientX - drag.startX) / drag.rect.width * 100; const dy = (event.clientY - drag.startY) / drag.rect.height * 100; patchArea(areaIndex, { xPercent: Math.max(0, Math.min(100 - printAreas[areaIndex].widthPercent, drag.x + dx)), yPercent: Math.max(0, Math.min(100 - printAreas[areaIndex].heightPercent, drag.y + dy)) }); } else if (resize) { const dx = (event.clientX - resize.startX) / rect.width * 100; const dy = (event.clientY - resize.startY) / rect.height * 100; patchArea(areaIndex, { widthPercent: Math.max(5, Math.min(100 - resize.x, resize.width + dx)), heightPercent: Math.max(5, Math.min(100 - resize.y, resize.height + dy)) }); } }} onPointerUp={() => { dragRef.current = null; resizeRef.current = null; }} onPointerLeave={() => { dragRef.current = null; resizeRef.current = null; }}>
          ...
          {activeScene?.mockupUrl || mockupUrl || previewImageUrl ? <img src={activeScene?.mockupUrl || mockupUrl || previewImageUrl} alt="Xem trước mockup" className="absolute inset-0 h-full w-full object-contain p-2" /> : <div className="absolute inset-0 grid place-items-center p-6 text-center text-xs text-slate-400">Thêm URL mockup nền trơn để xem trước vùng thiết kế.</div>}
