    ...
    <section id="product-personalizer" className={`overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm ${className}`}>
      <div className="border-b border-slate-200 bg-slate-950 px-4 py-3 text-white">
        <div className="flex items-center justify-between gap-4">
          <div className="flex min-w-0 items-center gap-2.5"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-orange-500"><Sparkles className="h-4 w-4" /></span><h3 className="truncate text-sm font-black">Cá nhân hoá sản phẩm</h3></div>
        ...
        <button type="button" onClick={onPreviewRequest} className="mc-focus-ring mt-3 inline-flex min-h-9 items-center gap-1.5 rounded-lg bg-white/10 px-2.5 text-[10px] font-bold text-slate-200 transition hover:bg-white/20"><Eye className="h-3.5 w-3.5" /> Xem mockup</button>
        ...
        <div className="order-2 space-y-4 overflow-visible p-4 sm:p-5 md:order-1">
          {fieldGroups.length > 0 ? fieldGroups.map((group, groupIndex) => {
            ...
            return <fieldset key={group.key} className={`rounded-xl border bg-slate-50/70 transition-colors ${isOpen ? "border-orange-200" : "border-slate-200"}`}>
              ...
              {isOpen && <div className="border-t border-slate-200 px-3 pb-3 pt-2.5 sm:px-4"><p className="mb-3 text-[10px] font-semibold text-slate-500">{requiredCount ? `${completedCount}/${requiredCount} mục bắt buộc` : "Tùy chọn"}</p><div className="space-y-4">{group.fields.map(renderField)}</div></div>}
            </fieldset>;
          ...
          {fieldGroups.length > 1 && <button type="button" onClick={() => setShowAllGroups(current => !current)} className="mc-focus-ring min-h-10 w-full rounded-xl border border-dashed border-slate-300 bg-white text-[11px] font-bold text-slate-600 hover:border-orange-400 hover:text-orange-700">{showAllGroups ? "Thu gọn" : "Mở rộng"}</button>}
          <div className="flex items-center gap-1.5 text-[10px] font-semibold text-emerald-700"><CheckCircle2 className="h-3.5 w-3.5" /> Tự động lưu</div>
        </div>
