...
export const StoreOrderTrackerModal: React.FC<StoreOrderTrackerModalProps> = ({
  ...
  const performSearch = async (searchStr: string) => {
    ...
    setSearched(true);
    try {
      const res = await AdminApi.trackStoreOrder(searchStr.trim());
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 animate-in fade-in duration-200">
      <div className="relative bg-white rounded-3xl shadow-2xl max-w-xl w-full border border-slate-200 overflow-hidden">
        {/* Header */}
        <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/80">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-sm text-slate-900">Tra Cứu Tình Trạng Đơn Hàng</h3>
              <p className="text-[11px] text-slate-500">Nhập mã đơn hàng hoặc số điện thoại người nhận</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
          ...
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Ví dụ: HUB-819230 hoặc 0912345678"
            ...
            <button
              ...
              className="px-4 py-2.5 bg-orange-600 hover:bg-orange-700 text-white rounded-xl text-xs font-bold transition-all disabled:bg-slate-300 flex items-center gap-1.5 shrink-0"
            >
              {isLoading ? (
          ...
          {searched && !isLoading && orders.length === 0 && !errorMessage && (
            <div className="text-center py-8 text-slate-400 space-y-2">
              ...
              <p className="text-xs">Không tìm thấy đơn hàng nào khớp với "{query}"</p>
            </div>
          )}
