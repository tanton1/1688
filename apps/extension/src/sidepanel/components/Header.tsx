...
import { Check, Globe, KeyRound, RefreshCw, Settings, ShieldCheck, X } from "lucide-react";
import { Raw1688Shop } from "@hub1688/shared-types";
import { DEFAULT_API_URL, getAccessToken, getApiBaseUrl, setAccessToken, setApiBaseUrl } from "../../shared/config.js";

interface HeaderProps { shop?: Raw1688Shop; onRefresh: () => void; loading: boolean; }
...
export const Header: React.FC<HeaderProps> = ({ shop, onRefresh, loading }) => {
  ...
  const save = async () => {
    ...
    if (accessTokenInput.trim()) {
      await setAccessToken(accessTokenInput);
      setAccessTokenInput("");
      setHasToken(true);
    }
    setSaved(true);
  ...
  return <header className="sticky top-0 z-50 border-b border-slate-200 bg-white px-4 py-3">
    ...
    {showSettings && <section aria-label="Cấu hình extension" className="mt-3 space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
      ...
      <label className="block text-xs font-semibold text-slate-700"><span className="mb-1 flex items-center gap-1"><KeyRound size={13} /> Access token</span><input type="password" autoComplete="off" value={accessTokenInput} onChange={event => setAccessTokenInput(event.target.value)} placeholder={hasToken ? "Đã lưu trong phiên — nhập để thay thế" : "Token do quản trị viên cấp"} className="w-full rounded-lg border border-slate-300 bg-white px-2.5 py-2 font-mono text-xs focus:border-orange-500 focus:outline-none" /></label>
      <div className="flex items-center justify-between rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 py-2 text-xs text-emerald-800"><span className="flex items-center gap-1.5"><ShieldCheck size={14} /> AI do máy chủ quản lý; extension không lưu API key.</span><span className="font-bold">{hasToken ? "Đã xác thực" : "Chưa có token"}</span></div>
      <div className="flex justify-end gap-2"><button type="button" onClick={async () => { await setAccessToken(""); setHasToken(false); }} className="rounded-lg px-3 py-2 text-xs font-semibold text-rose-700 hover:bg-rose-50">Xóa token</button><button type="button" onClick={save} className="flex items-center gap-1 rounded-lg bg-orange-600 px-3 py-2 text-xs font-bold text-white hover:bg-orange-700">{saved && <Check size={14} />}{saved ? "Đã lưu" : "Lưu"}</button></div>
    </section>}
